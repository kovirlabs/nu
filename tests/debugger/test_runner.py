...
def test_Debugger_output_stack_normal():
    ...
    db.stack = [(None, 1), (frame1, 2), (frame2, 3)]
    db.curframe = None
    db.output_stack()
    expected_stack = [
        (
...
def test_Debugger_output_stack_exception():
    ...
    db.stack = [(None, 1), (frame1, 2), (None, 3), (frame3, 4), (frame4, 5)]
    db.curframe = None
    db.output_stack()
    expected_stack = [
        (
...
def test_Debugger_setup():
    ...
    mock_stack = [None, (mock_curframe, 1)]
...
def test_Debugger_interact_exception_encountered():
    ...
    db.reset = mock.MagicMock()
    db.commands = mock.MagicMock()
    db.commands.get.side_effect = [("next", {}), ("quit", {"foo": "bar"})]
...
def test_Debugger_do_disable():
    ...
    db.output.assert_called_once_with("breakpoint_disable", bpnum=1)
...
def test_Debugger_do_clear():
    ...
    db.output.assert_called_once_with("breakpoint_clear", bpnum=1)
...
def test_Debugger_runscript():
    ...
    assert db._user_requested_quit
    db.run.assert_called_once_with(
        '__debug_script__ = open(r"x.py", "rb");'
