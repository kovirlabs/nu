...
The actual editor lives in `mu/`. The distribution is published as `nu-editor` (the fork's own PyPI identity, renamed from the inherited `mu-editor`); the version is **2.0.0b2** — nu's own line, major-bumped from upstream Mu 1.2.2 to mark the breaking Qt6/Python-3.11+ modernization (a continuation, not a reset). The importable package stays `mu` and the version lives in `mu/__init__.py`. The in-app/installer display name remains "Mu Editor".
